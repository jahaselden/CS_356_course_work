...
# Assuming the test cases directory is provided as the second argument
test_cases_directory="Test-Cases"

# Create a copy of the test cases directory
test_cases_copy=$(mktemp -d)
cp -R "$test_cases_directory"/* "$test_cases_copy"

# Use the copy for all operations
test_cases_directory="$test_cases_copy"
...
[ ! -f "$test_cases_directory"/empty.txt ] && touch "$test_cases_directory"/empty.txt
...
[ ! -z "$executable" ] && run_test_and_check_output "$cipher_type_validation_message" "$executable_path" A "$test_cases_directory"/Block-Cipher/1/bc-input-tc-1.txt out.txt "$test_cases_directory"/Block-Cipher/1/bc-key-tc-1.txt E
...
[ ! -z "$executable" ] && run_test_and_check_output "$mode_type_validation_message" "$executable_path" B "$test_cases_directory"/Block-Cipher/1/bc-input-tc-1.txt out.txt "$test_cases_directory"/Block-Cipher/1/bc-key-tc-1.txt X
...
[ ! -z "$executable" ] && run_test_and_check_output "$input_file_validation_message" "$executable_path" B xyz.txt out.txt "$test_cases_directory"/Block-Cipher/1/bc-key-tc-1.txt E
...
[ ! -z "$executable" ] && run_test_and_check_output "$key_file_validation_message" "$executable_path" B "$test_cases_directory"/Block-Cipher/1/bc-input-tc-1.txt out.txt xyz.txt E
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" B "$test_cases_directory"/empty.txt "$submission_directory"/b-e-output-for-empty-input.txt "$test_cases_directory"/Block-Cipher/1/bc-key-tc-1.txt E
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" B "$test_cases_directory"/empty.txt "$submission_directory"/b-d-output-for-empty-input.txt "$test_cases_directory"/Block-Cipher/1/bc-key-tc-1.txt D
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" S "$test_cases_directory"/empty.txt "$submission_directory"/s-e-output-for-empty-input.txt "$test_cases_directory"/Stream-Cipher/1/sc-key-tc-1.txt E
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" S "$test_cases_directory"/empty.txt "$submission_directory"/s-d-output-for-empty-input.txt "$test_cases_directory"/Stream-Cipher/1/sc-key-tc-1.txt D
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" B "$test_cases_directory"/Block-Cipher/1/bc-input-tc-1.txt "$submission_directory"/b-e-output-tc-1.txt "$test_cases_directory"/Block-Cipher/1/bc-key-tc-1.txt E

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Block Cipher Test Case 1: Encryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/b-e-output-tc-1.txt ] && diff -q "$test_cases_directory"/Block-Cipher/1/bc-encrypted-output-tc-1.txt "$submission_directory"/b-e-output-tc-1.txt > /dev/null; then
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" B "$test_cases_directory"/Block-Cipher/1/bc-encrypted-output-tc-1.txt "$submission_directory"/b-d-output-tc-1.txt "$test_cases_directory"/Block-Cipher/1/bc-key-tc-1.txt D

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Block Cipher Test Case 1: Decryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/b-d-output-tc-1.txt ] && diff -q "$test_cases_directory"/Block-Cipher/1/bc-decrypted-output-tc-1.txt "$submission_directory"/b-d-output-tc-1.txt > /dev/null; then
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" B "$test_cases_directory"/Block-Cipher/2/bc-input-tc-2.txt "$submission_directory"/b-e-output-tc-2.txt "$test_cases_directory"/Block-Cipher/2/bc-key-tc-2.txt E

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Block Cipher Test Case 2: Encryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/b-e-output-tc-2.txt ] && diff -q "$test_cases_directory"/Block-Cipher/2/bc-encrypted-output-tc-2.txt "$submission_directory"/b-e-output-tc-2.txt > /dev/null; then
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" B "$test_cases_directory"/Block-Cipher/2/bc-encrypted-output-tc-2.txt "$submission_directory"/b-d-output-tc-2.txt "$test_cases_directory"/Block-Cipher/2/bc-key-tc-2.txt D

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Block Cipher Test Case 2: Decryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/b-d-output-tc-2.txt ] && diff -q "$test_cases_directory"/Block-Cipher/2/bc-decrypted-output-tc-2.txt "$submission_directory"/b-d-output-tc-2.txt > /dev/null; then
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" B "$test_cases_directory"/Block-Cipher/3/bc-input-tc-3.txt "$submission_directory"/b-e-output-tc-3.txt "$test_cases_directory"/Block-Cipher/3/bc-key-tc-3.txt E

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Block Cipher Test Case 3: Encryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/b-e-output-tc-3.txt ] && diff -q "$test_cases_directory"/Block-Cipher/3/bc-encrypted-output-tc-3.txt "$submission_directory"/b-e-output-tc-3.txt > /dev/null; then
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" B "$test_cases_directory"/Block-Cipher/3/bc-encrypted-output-tc-3.txt "$submission_directory"/b-d-output-tc-3.txt "$test_cases_directory"/Block-Cipher/3/bc-key-tc-3.txt D

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Block Cipher Test Case 3: Decryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/b-d-output-tc-3.txt ] && diff -q "$test_cases_directory"/Block-Cipher/3/bc-decrypted-output-tc-3.txt "$submission_directory"/b-d-output-tc-3.txt > /dev/null; then
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" S "$test_cases_directory"/Stream-Cipher/1/sc-input-tc-1.txt "$submission_directory"/s-e-output-tc-1.txt "$test_cases_directory"/Stream-Cipher/1/sc-key-tc-1.txt E

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Stream Cipher Test Case 1: Encryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/s-e-output-tc-1.txt ] && diff -q "$test_cases_directory"/Stream-Cipher/1/sc-encrypted-output-tc-1.txt "$submission_directory"/s-e-output-tc-1.txt > /dev/null; then
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" S "$test_cases_directory"/Stream-Cipher/1/sc-encrypted-output-tc-1.txt "$submission_directory"/s-d-output-tc-1.txt "$test_cases_directory"/Stream-Cipher/1/sc-key-tc-1.txt D

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Stream Cipher Test Case 1: Decryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/s-d-output-tc-1.txt ] && diff -q "$test_cases_directory"/Stream-Cipher/1/sc-decrypted-output-tc-1.txt "$submission_directory"/s-d-output-tc-1.txt > /dev/null; then
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" S "$test_cases_directory"/Stream-Cipher/2/sc-input-tc-2.txt "$submission_directory"/s-e-output-tc-2.txt "$test_cases_directory"/Stream-Cipher/2/sc-key-tc-2.txt E

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Stream Cipher Test Case 2: Encryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/s-e-output-tc-2.txt ] && diff -q "$test_cases_directory"/Stream-Cipher/2/sc-encrypted-output-tc-2.txt "$submission_directory"/s-e-output-tc-2.txt > /dev/null; then
...
[ ! -z "$executable" ] && execute_and_check_for_crash "$executable_path" S "$test_cases_directory"/Stream-Cipher/2/sc-encrypted-output-tc-2.txt "$submission_directory"/s-d-output-tc-2.txt "$test_cases_directory"/Stream-Cipher/2/sc-key-tc-2.txt D

if [ $? -ne 0 ]; then
        echo -e "$cross_mark Stream Cipher Test Case 2: Decryption Failed Due to Segmentation Fault (Core Dumped) or An Error Occurred\n"
else
        if [ -f "$submission_directory"/s-d-output-tc-2.txt ] && diff -q "$test_cases_directory"/Stream-Cipher/2/sc-decrypted-output-tc-2.txt "$submission_directory"/s-d-output-tc-2.txt > /dev/null; then
...
rm -f *.txt *.o "$test_cases_directory"/empty.txt

# Clean up the test cases copy
rm -rf "$test_cases_copy"